"""Profile GPU rollout to identify bottlenecks."""
import sys, os
...
if __name__ == '__main__':
	...
	with torch.no_grad():
		...
		encoded = encode_states(state, ho)
		h = net(encoded)
	torch.cuda.synchronize()

	# Profile per-step components
	n_steps = 50
	...
	with torch.no_grad():
		for step in range(n_steps):
			...
			t0 = time.perf_counter()
			apply_actions(state, actions, ho, active & has_action)
			torch.cuda.synchronize()
			times['apply'] += time.perf_counter() - t0

	actual_steps = min(step + 1, n_steps)
	total = sum(times.values())
	print(f"B={B}, {actual_steps} steps, total={total:.3f}s ({total/actual_steps*1000:.1f}ms/step)\n")
